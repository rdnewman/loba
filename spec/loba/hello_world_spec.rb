...
RSpec.describe HelloWorld do
  ...
  describe '#hello' do
    it 'writes a first timestamp' do
      expected_output = /\[TIMESTAMP\].*#=.*, diff=.*, at=.*in\s.*.rb:7:in `initialize'/
      expect { hello_world.hello }.to output(expected_output).to_stdout
    end

    it 'writes a value notice' do
      expected_output = /\[HelloWorld#hello\].*@x:.*42.*\(in\s.*.rb:12:in `hello'/
      expect { hello_world.hello }.to output(expected_output).to_stdout
    end

    it 'writes a second timestamp' do
      expected_output = /\[TIMESTAMP\].*#=.*, diff=.*, at=.*in\s.*.rb:14:in `hello'/
      expect { hello_world.hello }.to output(expected_output).to_stdout
    end
  end
  ...
  describe '#goodbye' do
    it 'writes a value notice using a label' do
      expected_output = /\[HelloWorld#goodbye\].*@y:.*Charlie.*\(in\s.*.rb:18:in `goodbye'/
